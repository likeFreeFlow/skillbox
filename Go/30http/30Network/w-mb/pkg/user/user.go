...
func (u User) AddFriend(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		check.ReturnIntServErr(&w, check.Err{"Не найден пользователь отправляющий запрос"})
		return
	}
	u2, ok := UserStorage.Search(nFriends.Id2)
	if !ok {
		check.ReturnIntServErr(&w, check.Err{"Не найден пользователь принимающий запрос"})
		return
	}
